...
{
    emEthDbgMsg(pDemoThreadParam->dwMasterID, byEthTypeByte0, byEthTypeByte1, szMsg);
}
...
EC_T_BOOL PrintSlaveInfos(T_DEMO_THREAD_PARAM* pDemoThreadParam)
{
    ...
    EC_T_CFG_SLAVE_INFO oCfgSlaveInfo;
        ...
        dwRes = emGetBusSlaveInfo(pDemoThreadParam->dwMasterID, EC_FALSE, wAutoIncAddress, &oBusSlaveInfo);
        ...
        EcLogMsg(EC_LOG_LEVEL_INFO, (pEcLogContext, EC_LOG_LEVEL_INFO, "Bus AutoInc Address.: 0x%04x\n", oBusSlaveInfo.wAutoIncAddress, oBusSlaveInfo.wAutoIncAddress));
        ...
        dwRes = emGetCfgSlaveInfo(pDemoThreadParam->dwMasterID, EC_TRUE, oBusSlaveInfo.wStationAddress, &oCfgSlaveInfo);
        if (EC_E_NOERROR != dwRes)
        {
            continue;
        }
...
EC_T_VOID PrintCfgSlavesInfo(T_DEMO_THREAD_PARAM* pDemoThreadParam)
{
    ...
    for (wAutoIncAddress = 0; wAutoIncAddress != 1; wAutoIncAddress--)
    {
        ...
        dwRes = emGetCfgSlaveInfo(pDemoThreadParam->dwMasterID, EC_FALSE, wAutoIncAddress, &oCfgSlaveInfo);
        if (EC_E_NOERROR != dwRes)
        {
            break;
        }
    ...
    ,EC_T_WORD*           pwPhysAddr)         /**< [out]  Physical Address of slave */
{
    ...
    for (dwSlaveIdx = 0; dwSlaveIdx < emGetNumConnectedSlaves(pDemoThreadParam->dwMasterID); dwSlaveIdx++)
    {
    ...
    ,EC_T_WORD*           pwPhysAddr)         /**< [out]  Physical Address of slave */
{
    ...
    for (dwSlaveIdx = 0; dwSlaveIdx < emGetNumConfiguredSlaves(pDemoThreadParam->dwMasterID); dwSlaveIdx++)
    {
        EC_T_WORD           wAutoIncAddress = (EC_T_WORD)(0 - dwSlaveIdx);
        EC_T_CFG_SLAVE_INFO oCfgSlaveInfo;
...
EC_T_CHAR* SlaveVendorText(T_eEtherCAT_Vendor EV       /**< [in]   Vendor ID */)
{
    ...
    VENDOR_TEXT(ecvendor_omron,                              "OMRON Corporation");//WZZ
...
//	    case ecprodcode_omron_g5:            pRet = (EC_T_CHAR*)"G5-R88D-KN02H-ECT servo"; break;//wzz 
            default:                             pRet = (EC_T_CHAR*)"Unknown"; break;
            };
        } break;
#endif /*#if !(defined EC_DEMO_TINY)*/
...
#define MAX_OBNAME_LEN          ((EC_T_DWORD)100)
    ...
    pMbxGetODLTfer = emMbxTferCreate(pDemoThreadParam->dwMasterID, &MbxTferDesc);
    ...
    pMbxGetObDescTfer = emMbxTferCreate(pDemoThreadParam->dwMasterID, &MbxTferDesc);
    ...
    pMbxGetEntryDescTfer = emMbxTferCreate(pDemoThreadParam->dwMasterID, &MbxTferDesc);
    ...
    dwRes = emCoeGetODList(pDemoThreadParam->dwMasterID, pMbxGetODLTfer, dwNodeId, eODListType_ALL, dwTimeout);
    ...
    dwRes = emCoeGetODList(pDemoThreadParam->dwMasterID, pMbxGetODLTfer, dwNodeId, eODListType_RxPdoMap, dwTimeout);
    if (EC_E_SLAVE_NOT_PRESENT == dwRes)
    {
        dwRetVal = dwRes;
        goto Exit;
    ...
    dwRes = emCoeGetODList(pDemoThreadParam->dwMasterID, pMbxGetODLTfer, dwNodeId, eODListType_TxPdoMap, dwTimeout);
    if (EC_E_SLAVE_NOT_PRESENT == dwRes)
    {
        dwRetVal = dwRes;
        goto Exit;
    ...
    for (wIndex = 0; (wIndex < wODListLen) && !*pbStopReading; wIndex++)
    {
        ...
        /* get object description */
        dwRes = emCoeGetObjectDesc(pDemoThreadParam->dwMasterID, pMbxGetObDescTfer, dwNodeId, pwODList[wIndex], dwTimeout);
        if (EC_E_SLAVE_NOT_PRESENT == dwRes)
        {
            dwRetVal = dwRes;
            goto Exit;
        ...
        for (wSubIndex = 0; wSubIndex < wSubIndexLimit; wSubIndex++)
        {
            ...
            dwRes = emCoeGetEntryDesc(pDemoThreadParam->dwMasterID, pMbxGetEntryDescTfer, dwNodeId, pwODList[wIndex], EC_LOBYTE(wSubIndex), byValueInfoType, dwTimeout);
            if (EC_E_SLAVE_NOT_PRESENT == dwRes)
            {
                dwRetVal = dwRes;
                goto Exit;
            ...
            if (bPerformUpload)
            {
                ...
                dwRes = emCoeSdoUpload(
                    pDemoThreadParam->dwMasterID, dwNodeId, pwODList[wIndex], EC_LOBYTE(wSubIndex),
...
Exit:
    ...
    if (EC_NULL != pMbxGetODLTfer)
    {
        emMbxTferDelete(pDemoThreadParam->dwMasterID, pMbxGetODLTfer);
        pMbxGetODLTfer = EC_NULL;
    }
    if (EC_NULL != pMbxGetObDescTfer)
    {
        emMbxTferDelete(pDemoThreadParam->dwMasterID, pMbxGetObDescTfer);
        pMbxGetObDescTfer = EC_NULL;
    }
    if (EC_NULL != pMbxGetEntryDescTfer)
    {
        emMbxTferDelete(pDemoThreadParam->dwMasterID, pMbxGetEntryDescTfer);
