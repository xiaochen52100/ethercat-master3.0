...
CFrameLogMultiplexer CFrameLogMultiplexer::G_aLogMultiplexer[];
#endif
...
EC_T_BOOL CAtEmLogging::s_bLogFileEnb = EC_FALSE;
#else
EC_T_BOOL CAtEmLogging::s_bLogFileEnb = EC_TRUE;
#endif
...
#if (defined INCLUDE_FILE_LOGGING)
                if (s_bLogFileEnb && (pMsgBufferDesc->pfMsgFile != EC_NULL))
                {
...
CFrameLogMultiplexer::CFrameLogMultiplexer()
{
    ...
    for (int i = 0; i < MAX_NUMOF_MASTER_INSTANCES; i++)
    {
        CFrameLogMultiplexer::G_aLogMultiplexer[i].m_dwMasterId = i;
    }
}
...
EC_T_VOID CFrameSpy::Install(EC_T_DWORD dwMasterId)
{
    ...
    {
        CFrameLogMultiplexer::G_aLogMultiplexer[dwMasterId].AddFrameLogger((EC_T_VOID*)this, StaticFrameHandler);
        m_dwMasterId = dwMasterId;
    }
}
...
EC_T_VOID CFrameSpy::Uninstall(EC_T_VOID)
{
    ...
    {
        CFrameLogMultiplexer::G_aLogMultiplexer[m_dwMasterId].RemoveFrameLogger((EC_T_VOID*)this, StaticFrameHandler);
        m_dwMasterId = 0xffff;
    }
}
...
#if ATECAT_VERSION < 0x03000000 /* V3.x */
    if (ETHERNET_FRAME_TYPE_BKHF != EC_ETHFRM_GET_FRAMETYPE(pbyFrame)) return;
#else
    if (ETHERNET_FRAME_TYPE_BKHF != EC_ETHFRM_GET_FRAMETYPE(&pbyFrame[2 * ETHERNET_ADDRESS_LEN])) return;
#endif
...
EC_T_VOID CPcapRecorder::Install(EC_T_DWORD dwMasterId)
{
    ...
    if (m_dwMasterId != dwMasterId)
    {
        CFrameLogMultiplexer::G_aLogMultiplexer[dwMasterId].AddFrameLogger((EC_T_VOID*)this, LogFrameStatic);
        m_dwMasterId = dwMasterId;
    }
}
...
EC_T_VOID CPcapRecorder::Uninstall()
{
    if (0xffff != m_dwMasterId)
    {
        CFrameLogMultiplexer::G_aLogMultiplexer[m_dwMasterId].RemoveFrameLogger((EC_T_VOID*)this, LogFrameStatic);
        m_dwMasterId = 0xffff;
    }
}
